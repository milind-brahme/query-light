...
class ChildTableWithParent ( var childTab:String, val childTabAlias:String )
...
fun JdbcPersistent.buildWhereClause( sqlStmt : String) : String
{
    ...
    val ctw:ChildTableWithParent = ChildTableWithParent(tableName,alias)
...
class ChangeTracker  (  var j : JdbcPersistent  ) {
    ...
    var provider : CompletionProvider = DefaultCompletionProvider()
     ...
     fun trackChanges(sqlStmt: String) {

        var changeCounter  = 0;
        var tableListString: String = regexTableList.find(sqlStmt)?.groups?.get(1)?.value ?: ""
   ...
   private fun  updateColumnCompletionProvider(  ) : CompletionProvider
    {
        ...
        val listTableSql= "select column_name from all_Tab_columns where table_name in (" +  tmp.joinToString () + ")"
        val provider = DefaultCompletionProvider()
